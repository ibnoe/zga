...
class sellable_itemview extends Controller {

	function sellable_itemview()
	{
		parent::Controller();	
...
$this->db->where('id', $sellable_item_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('sellable_item_view_form', $data);
}
...
$this->db->where('id', $data['sellable_item_id']);
