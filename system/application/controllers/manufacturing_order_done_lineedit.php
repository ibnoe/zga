...
class manufacturing_order_done_lineedit extends Controller {

	function manufacturing_order_done_lineedit()
	{
		parent::Controller();	
	...
	function index($manufacturing_order_done_line_id=0)
	{
		if ($manufacturing_order_done_line_id == 0)
		{
...
$q = $this->db->where('id', $manufacturing_order_done_line_id);
$this->db->select('*');
$q = $this->db->get('manufacturingorderdoneline');
if ($q->num_rows() > 0) {
$data = array();
$data['manufacturing_order_done_line_id'] = $manufacturing_order_done_line_id;
...
$this->load->view('manufacturing_order_done_line_edit_form', $data);
}
...
$this->db->where('id', $_POST['manufacturing_order_done_line_id']);
$this->db->update('manufacturingorderdoneline', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('manufacturing_order_done_lineedit','manufacturingorderdoneline','afteredit', $_POST['manufacturing_order_done_line_id']);
