...
$this->db->select('item.updatedby as item__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "item.idstring like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || itemcategory.name like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || item.minquantity like '%".$_POST['searchtext']."%'";$where .= " || item.maxquantity like '%".$_POST['searchtext']."%'";$where .= " || item.buffer3months like '%".$_POST['searchtext']."%'";$where .= " || item.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || item.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('item__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		$data['fields'] = array('item__idstring' => 'Item ID', 'item__name' => 'Name', 'itemcategory__name' => 'Category', 'coa__name' => 'Account Persediaan', 'coa1__name' => 'Account HPP', 'item__minquantity' => 'Minimum Quantity', 'item__maxquantity' => 'Maximum Quantity', 'item__buffer3months' => 'Buffer 3 Months', 'item__lastupdate' => 'Last Update', 'item__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('item_lookup_view', $data);
	}
}
