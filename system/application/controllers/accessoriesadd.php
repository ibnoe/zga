...
class accessoriesadd extends Controller {
	...
	function index($id = 0)
	{
		...
		$data['item__idstring'] = $this->generallib->genId('Accessories');
		$data['item__name'] = '';
		...
		$data['item__intitemtype'] = 'accessories';
		$data['item__itemcategory_id'] = '10';
		...
		$this->load->view('accessories_add_form', $data);
	}
	...
	function submit()
	{
		...
		if (isset($_POST['item__idstring'])) {
			...
			if ($error == "")
			{
				...
				$error .= $this->generallib->commonfunction('accessoriesadd','item','aftersave', $item_id);
