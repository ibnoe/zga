...
class delivery_order_for_invoiceedit extends Controller {

	function delivery_order_for_invoiceedit()
	{
		parent::Controller();	
	...
	function index($delivery_order_for_invoice_id=0)
	{
		if ($delivery_order_for_invoice_id == 0)
		{
...
$q = $this->db->where('id', $delivery_order_for_invoice_id);
$this->db->select('*');
...
if ($q->num_rows() > 0) {
$data = array();
$data['delivery_order_for_invoice_id'] = $delivery_order_for_invoice_id;
...
$this->load->view('delivery_order_for_invoice_edit_form', $data);
}
...
if (isset($_POST['deliveryorder__orderid'])) {$this->db->where("id !=", $_POST['delivery_order_for_invoice_id']);
...
$this->db->where('id', $_POST['delivery_order_for_invoice_id']);
$this->db->update('deliveryorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('delivery_order_for_invoiceedit','deliveryorder','afteredit', $_POST['delivery_order_for_invoice_id']);
