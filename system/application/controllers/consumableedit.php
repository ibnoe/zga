...
class consumableedit extends Controller {

	function consumableedit()
	{
		parent::Controller();	
	...
	function index($consumable_id=0)
	{
		if ($consumable_id == 0)
		{
...
$q = $this->db->where('id', $consumable_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('consumable_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['consumable_id']);
...
$data['subcategory'] = $_POST['item__subcategory'];if (isset($_POST['item__brand']))
$data['brand'] = $_POST['item__brand'];if (isset($_POST['item__minquantity']))
...
$this->db->where('id', $_POST['consumable_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('consumableedit','item','afteredit', $_POST['consumable_id']);
