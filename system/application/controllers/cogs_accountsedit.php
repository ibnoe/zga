...
class cogs_accountsedit extends Controller {

	function cogs_accountsedit()
	{
		parent::Controller();	
	...
	function index($cogs_accounts_id=0)
	{
		if ($cogs_accounts_id == 0)
		{
...
$q = $this->db->where('id', $cogs_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
$data['cogs_accounts_id'] = $cogs_accounts_id;
...
$this->load->view('cogs_accounts_edit_form', $data);
}
...
if (isset($_POST['coa__idstring'])) {$this->db->where("id !=", $_POST['cogs_accounts_id']);
...
$this->db->where('id', $_POST['cogs_accounts_id']);
$this->db->update('coa', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('cogs_accountsedit','coa','afteredit', $_POST['cogs_accounts_id']);
