...
class chemical_manufacturing_orderlist extends Controller {

	function chemical_manufacturing_orderlist()
	{
		parent::Controller();	
...
$this->db->join('penambahanstockchemical', 'penambahanstockchemical.id = manufacturingorder.penambahanstockchemical_id', 'left');
...
$this->db->select('manufacturingorder.penambahanstockchemical_id as manufacturingorder__penambahanstockchemical_id', false);
...
$this->db->select('manufacturingorder.updatedby as manufacturingorder__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "manufacturingorder.idstring like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.date like '%".$_POST['searchtext']."%'";$where .= " || penambahanstockchemical.idstring like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse1.name like '%".$_POST['searchtext']."%'";$where .= " || bom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.quantity like '%".$_POST['searchtext']."%'";$where .= " || uom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('manufacturingorder__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		///
		$this->load->view('chemical_manufacturing_order_list_view', $data);
	}
}
