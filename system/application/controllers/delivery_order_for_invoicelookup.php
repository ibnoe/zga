...
class delivery_order_for_invoicelookup extends Controller {

	function delivery_order_for_invoicelookup()
	{
		parent::Controller();	
...
$this->db->join('salesinvoice', 'deliveryorder.id = salesinvoice.deliveryorder_id', 'left');
$this->db->where('deliveryorder.disabled = 0');
$this->db->where('salesinvoice.deliveryorder_id is NULL');
...
$this->db->order_by('deliveryorder__date', 'asc');
$this->db->order_by('deliveryorder__date', 'desc');
$this->db->order_by('deliveryorder__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		///
		$this->load->view('delivery_order_for_invoice_lookup_view', $data);
	}
}
