...
class sales_return_order_open_paymentedit extends Controller {

	function sales_return_order_open_paymentedit()
	{
		parent::Controller();	
	...
	function index($sales_return_order_open_payment_id=0)
	{
		if ($sales_return_order_open_payment_id == 0)
		{
...
$q = $this->db->where('id', $sales_return_order_open_payment_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('salesreturnorder');
if ($q->num_rows() > 0) {
$data = array();
$data['sales_return_order_open_payment_id'] = $sales_return_order_open_payment_id;
...
$this->load->view('sales_return_order_open_payment_edit_form', $data);
}
...
if (isset($_POST['salesreturnorder__salesreturnorderid'])) {$this->db->where("id !=", $_POST['sales_return_order_open_payment_id']);
...
$this->db->where('id', $_POST['sales_return_order_open_payment_id']);
$this->db->update('salesreturnorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('sales_return_order_open_paymentedit','salesreturnorder','afteredit', $_POST['sales_return_order_open_payment_id']);
