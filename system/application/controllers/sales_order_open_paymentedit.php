...
class sales_order_open_paymentedit extends Controller {

	function sales_order_open_paymentedit()
	{
		parent::Controller();	
	...
	function index($sales_order_open_payment_id=0)
	{
		if ($sales_order_open_payment_id == 0)
		{
...
$q = $this->db->where('id', $sales_order_open_payment_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('salesorder');
if ($q->num_rows() > 0) {
$data = array();
$data['sales_order_open_payment_id'] = $sales_order_open_payment_id;
...
$this->load->view('sales_order_open_payment_edit_form', $data);
}
...
if (isset($_POST['salesorder__orderid'])) {$this->db->where("id !=", $_POST['sales_order_open_payment_id']);
...
$this->db->where('id', $_POST['sales_order_open_payment_id']);
$this->db->update('salesorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('sales_order_open_paymentedit','salesorder','afteredit', $_POST['sales_order_open_payment_id']);
