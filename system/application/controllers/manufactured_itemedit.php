...
class manufactured_itemedit extends Controller {

	function manufactured_itemedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $manufactured_item_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('manufactured_item_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['manufactured_item_id']);
...
$this->db->where('id', $_POST['manufactured_item_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('manufactured_itemedit','item','afteredit', $_POST['manufactured_item_id']);
