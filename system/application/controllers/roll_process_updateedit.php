...
class roll_process_updateedit extends Controller {

	function roll_process_updateedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $roll_process_update_id);
$this->db->select('*');
...
$this->load->view('roll_process_update_edit_form', $data);
}
...
if (isset($_POST['rollprocessupdate__idstring'])) {$this->db->where("id !=", $_POST['roll_process_update_id']);
...
if (isset($_POST['rollprocessupdate__noorderandcustomer'])) {$this->db->where("id !=", $_POST['roll_process_update_id']);
...
$this->db->where('id', $_POST['roll_process_update_id']);
$this->db->update('rollprocessupdate', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('roll_process_updateedit','rollprocessupdate','afteredit', $_POST['roll_process_update_id']);
