...
class chemicallist extends Controller {
...
$this->db->select('item.updatedby as item__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "item.idstring like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || item.chemicalcode like '%".$_POST['searchtext']."%'";$where .= " || item.chemicaltype like '%".$_POST['searchtext']."%'";$where .= " || item.packingsize like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || itemcategory.name like '%".$_POST['searchtext']."%'";$where .= " || item.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || item.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('item__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		$data['fields'] = array('item__idstring' => 'Item ID', 'item__name' => 'Name', 'item__chemicalcode' => 'Product Code', 'item__chemicaltype' => 'Product Classification', 'item__packingsize' => 'Packing Size', 'coa__name' => 'Account Persediaan', 'coa1__name' => 'Account HPP', 'itemcategory__name' => 'Category', 'item__lastupdate' => 'Last Update', 'item__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('chemical_list_view', $data);
	}
}
