...
class supplier_3edit extends Controller {

	function supplier_3edit()
	{
		parent::Controller();	
	...
	function index($supplier_3_id=0)
	{
		if ($supplier_3_id == 0)
		{
...
$q = $this->db->where('id', $supplier_3_id);
$this->db->select('*');
$q = $this->db->get('supplier');
if ($q->num_rows() > 0) {
$data = array();
$data['supplier_3_id'] = $supplier_3_id;
...
$this->load->view('supplier_3_edit_form', $data);
}
...
if (isset($_POST['supplier__idstring'])) {$this->db->where("id !=", $_POST['supplier_3_id']);
...
$this->db->where('id', $_POST['supplier_3_id']);
$this->db->update('supplier', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('supplier_3edit','supplier','afteredit', $_POST['supplier_3_id']);
