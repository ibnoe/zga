...
class inklist extends Controller {
...
$this->db->where('item.intitemtype = "ink"');
...
$this->db->select('item.inkcode as item__inkcode', false);
$this->db->select('item.weight as item__weight', false);
...
$this->db->select('item.updatedby as item__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "item.idstring like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || item.inkcode like '%".$_POST['searchtext']."%'";$where .= " || item.weight like '%".$_POST['searchtext']."%'";$where .= " || item.minquantity like '%".$_POST['searchtext']."%'";$where .= " || item.maxquantity like '%".$_POST['searchtext']."%'";$where .= " || item.buffer3months like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || itemcategory.name like '%".$_POST['searchtext']."%'";$where .= " || item.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || item.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('item__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		$data['fields'] = array('item__idstring' => 'Item ID', 'item__name' => 'Name', 'item__inkcode' => 'Ink Code', 'item__weight' => 'Weight', 'item__minquantity' => 'Minimum Quantity', 'item__maxquantity' => 'Maximum Quantity', 'item__buffer3months' => 'Buffer 3 Months', 'coa__name' => 'Account Persediaan', 'coa1__name' => 'Account HPP', 'itemcategory__name' => 'Category', 'item__lastupdate' => 'Last Update', 'item__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('ink_list_view', $data);
	}
}
