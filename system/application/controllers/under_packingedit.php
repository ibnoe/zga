...
class under_packingedit extends Controller {

	function under_packingedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $under_packing_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('under_packing_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['under_packing_id']);
...
$this->db->where('id', $_POST['under_packing_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('under_packingedit','item','afteredit', $_POST['under_packing_id']);
