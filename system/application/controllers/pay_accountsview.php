...
class pay_accountsview extends Controller {

	function pay_accountsview()
	{
		parent::Controller();	
...
$this->db->where('id', $pay_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('pay_accounts_view_form', $data);
}
...
$this->db->where('id', $data['pay_accounts_id']);
