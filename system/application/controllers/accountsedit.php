...
class accountsedit extends Controller {
...
$q = $this->db->where('id', $accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('accounts_edit_form', $data);
}
...
if (isset($_POST['coa__idstring'])) {$this->db->where("id !=", $_POST['accounts_id']);
...
$this->db->where('id', $_POST['accounts_id']);
$this->db->update('coa', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('accountsedit','coa','afteredit', $_POST['accounts_id']);
