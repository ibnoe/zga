...
class sales_invoice_lineedit extends Controller {

	function sales_invoice_lineedit()
	{
		parent::Controller();	
	...
	function index($sales_invoice_line_id=0)
	{
		if ($sales_invoice_line_id == 0)
		{
...
$q = $this->db->where('id', $sales_invoice_line_id);
$q = $this->db->get('salesinvoiceline');
if ($q->num_rows() > 0) {
$data = array();
$data['sales_invoice_line_id'] = $sales_invoice_line_id;
...
$this->load->view('sales_invoice_line_edit_form', $data);
}
...
$this->db->where('id', $_POST['sales_invoice_line_id']);
$this->db->update('salesinvoiceline', $data);
$this->load->library('generallib');
$this->generallib->commonfunction('sales_invoice_lineedit','salesinvoiceline','afteredit', $_POST['sales_invoice_line_id']);
