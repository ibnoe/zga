...
class repackingedit extends Controller {

	function repackingedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $repacking_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('manufacturingorder');
if ($q->num_rows() > 0) {
$data = array();
$data['repacking_id'] = $repacking_id;
...
$this->load->view('repacking_edit_form', $data);
}
...
if (isset($_POST['manufacturingorder__idstring'])) {$this->db->where("id !=", $_POST['repacking_id']);
...
$this->db->where('id', $_POST['repacking_id']);
$this->db->update('manufacturingorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('repackingedit','manufacturingorder','afteredit', $_POST['repacking_id']);
