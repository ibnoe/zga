...
class purchase_return_delivery_line_viewedit extends Controller {

	function purchase_return_delivery_line_viewedit()
	{
		parent::Controller();	
	...
	function index($purchase_return_delivery_line_view_id=0)
	{
		if ($purchase_return_delivery_line_view_id == 0)
		{
...
$q = $this->db->where('id', $purchase_return_delivery_line_view_id);
$this->db->select('*');
$q = $this->db->get('purchasereturndeliveryline');
if ($q->num_rows() > 0) {
$data = array();
$data['purchase_return_delivery_line_view_id'] = $purchase_return_delivery_line_view_id;
...
$this->load->view('purchase_return_delivery_line_view_edit_form', $data);
}
...
$this->db->where('id', $_POST['purchase_return_delivery_line_view_id']);
$this->db->update('purchasereturndeliveryline', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('purchase_return_delivery_line_viewedit','purchasereturndeliveryline','afteredit', $_POST['purchase_return_delivery_line_view_id']);
