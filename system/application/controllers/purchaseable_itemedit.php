...
class purchaseable_itemedit extends Controller {

	function purchaseable_itemedit()
	{
		parent::Controller();	
	...
	function index($purchaseable_item_id=0)
	{
		if ($purchaseable_item_id == 0)
		{
...
$q = $this->db->where('id', $purchaseable_item_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
$data['purchaseable_item_id'] = $purchaseable_item_id;
...
$this->load->view('purchaseable_item_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['purchaseable_item_id']);
...
$this->db->where('id', $_POST['purchaseable_item_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('purchaseable_itemedit','item','afteredit', $_POST['purchaseable_item_id']);
