...
class sales_payment_linelist extends Controller {

	function sales_payment_linelist()
	{
		parent::Controller();	
	...
	function _qhelp($data, $salespayment_id)
	{
		
$this->db->where('salespaymentline.salespayment_id', $salespayment_id);$this->db->from('salespaymentline');
...
$this->db->order_by('salespaymentline__lastupdate', 'desc');
	...
	function index($salespayment_id=0)
	{
		$data = array();
		...
		$data['foreign_id'] = $salespayment_id;$data['fields'] = array('salesinvoice__orderid' => 'Sales Invoice', 'salespaymentline__lastupdate' => 'Last Update', 'salespaymentline__updatedby' => 'Last Update By');
		...
		$data = $this->_qhelp($data, $salespayment_id);
		...
		///
		$this->load->view('sales_payment_line_list_view', $data);
	}
}
