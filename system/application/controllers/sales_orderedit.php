...
class sales_orderedit extends Controller {

	function sales_orderedit()
	{
		parent::Controller();	
	...
	function index($sales_order_id=0)
	{
		if ($sales_order_id == 0)
		{
...
$q = $this->db->where('id', $sales_order_id);
$this->db->select('*');
...
$this->load->view('sales_order_edit_form', $data);
}
...
if (isset($_POST['salesorder__orderid'])) {$this->db->where("id !=", $_POST['sales_order_id']);
...
$this->db->where('id', $_POST['sales_order_id']);
$this->db->update('salesorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('sales_orderedit','salesorder','afteredit', $_POST['sales_order_id']);
