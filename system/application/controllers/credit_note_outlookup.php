...
class credit_note_outlookup extends Controller {

	function credit_note_outlookup()
	{
		parent::Controller();	
...
$this->db->select('creditnoteout.updatedby as creditnoteout__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "creditnoteout.creditnoteoutid like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.date like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.expirydate like '%".$_POST['searchtext']."%'";$where .= " || customer.firstname like '%".$_POST['searchtext']."%'";$where .= " || coa.name like '%".$_POST['searchtext']."%'";$where .= " || currency.name like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.amount like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.amountused like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.notes like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.usedflag like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || creditnoteout.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('creditnoteout__creditnoteoutid', 'asc');
...
$this->db->order_by('creditnoteout__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		///
		$this->load->view('credit_note_out_lookup_view', $data);
	}
}
