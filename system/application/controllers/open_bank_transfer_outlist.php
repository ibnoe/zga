...
class open_bank_transfer_outlist extends Controller {

	function open_bank_transfer_outlist()
	{
		parent::Controller();	
...
$this->db->where('banktransferkeluar.transferedflag = 0');
...
$this->db->select('banktransferkeluar.id as banktransferkeluar__id', false);
...
$this->db->select('banktransferkeluar.updatedby as banktransferkeluar__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "banktransferkeluar.id like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.idstring like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.date like '%".$_POST['searchtext']."%'";$where .= " || currency.name like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.amount like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.notes like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.transferedflag like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || banktransferkeluar.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('banktransferkeluar__id', 'asc');
...
$this->db->order_by('banktransferkeluar__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		///
		$this->load->view('open_bank_transfer_out_list_view', $data);
	}
}
