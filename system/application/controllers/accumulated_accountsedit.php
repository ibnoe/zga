...
class accumulated_accountsedit extends Controller {

	function accumulated_accountsedit()
	{
		parent::Controller();	
	...
	function index($accumulated_accounts_id=0)
	{
		if ($accumulated_accounts_id == 0)
		{
...
$q = $this->db->where('id', $accumulated_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
$data['accumulated_accounts_id'] = $accumulated_accounts_id;
...
$this->load->view('accumulated_accounts_edit_form', $data);
}
...
if (isset($_POST['coa__idstring'])) {$this->db->where("id !=", $_POST['accumulated_accounts_id']);
...
$this->db->where('id', $_POST['accumulated_accounts_id']);
$this->db->update('coa', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('accumulated_accountsedit','coa','afteredit', $_POST['accumulated_accounts_id']);
