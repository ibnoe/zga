...
class purchase_return_orderedit extends Controller {

	function purchase_return_orderedit()
	{
		parent::Controller();	
	...
	function index($purchase_return_order_id=0)
	{
		if ($purchase_return_order_id == 0)
		{
...
$q = $this->db->where('id', $purchase_return_order_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('purchasereturnorder');
if ($q->num_rows() > 0) {
$data = array();
$data['purchase_return_order_id'] = $purchase_return_order_id;
...
$data['purchasereturnorder__created'] = $r->created;
$data['purchasereturnorder__createdby'] = $r->createdby;}
$this->load->view('purchase_return_order_edit_form', $data);
}
...
if (isset($_POST['purchasereturnorder__purchasereturnorderid'])) {$this->db->where("id !=", $_POST['purchase_return_order_id']);
...
$this->db->where('id', $_POST['purchase_return_order_id']);
$this->db->update('purchasereturnorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('purchase_return_orderedit','purchasereturnorder','afteredit', $_POST['purchase_return_order_id']);
