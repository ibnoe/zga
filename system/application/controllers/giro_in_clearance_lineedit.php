...
class giro_in_clearance_lineedit extends Controller {

	function giro_in_clearance_lineedit()
	{
		parent::Controller();	
	...
	function index($giro_in_clearance_line_id=0)
	{
		if ($giro_in_clearance_line_id == 0)
		{
...
$q = $this->db->where('id', $giro_in_clearance_line_id);
$this->db->select('*');
$q = $this->db->get('giroinclearanceline');
if ($q->num_rows() > 0) {
$data = array();
$data['giro_in_clearance_line_id'] = $giro_in_clearance_line_id;
...
$this->load->view('giro_in_clearance_line_edit_form', $data);
}
...
$this->db->where('id', $_POST['giro_in_clearance_line_id']);
$this->db->update('giroinclearanceline', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('giro_in_clearance_lineedit','giroinclearanceline','afteredit', $_POST['giro_in_clearance_line_id']);
