...
class blanket_manufacturing_orderlist extends Controller {

	function blanket_manufacturing_orderlist()
	{
		parent::Controller();	
...
$this->db->join('bif', 'bif.id = manufacturingorder.bif_id', 'left');
...
$this->db->select('manufacturingorder.bif_id as manufacturingorder__bif_id', false);
...
$this->db->select('manufacturingorder.updatedby as manufacturingorder__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "manufacturingorder.idstring like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.date like '%".$_POST['searchtext']."%'";$where .= " || bif.idstring like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse1.name like '%".$_POST['searchtext']."%'";$where .= " || bom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.quantity like '%".$_POST['searchtext']."%'";$where .= " || uom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('manufacturingorder__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		$data['fields'] = array('manufacturingorder__idstring' => 'ID', 'manufacturingorder__date' => 'Date', 'bif__idstring' => 'Blanket Identification Form', 'item__name' => 'Blanket', 'warehouse__name' => 'Raw Material Location', 'warehouse1__name' => 'Finish Goods Location', 'bom__name' => 'Bill Of Material', 'manufacturingorder__quantity' => 'Quantity', 'uom__name' => 'Unit', 'manufacturingorder__lastupdate' => 'Last Update', 'manufacturingorder__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('blanket_manufacturing_order_list_view', $data);
	}
}
