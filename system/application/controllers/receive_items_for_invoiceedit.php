...
class receive_items_for_invoiceedit extends Controller {

	function receive_items_for_invoiceedit()
	{
		parent::Controller();	
	...
	function index($receive_items_for_invoice_id=0)
	{
		if ($receive_items_for_invoice_id == 0)
		{
...
$q = $this->db->where('id', $receive_items_for_invoice_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('receiveditem');
if ($q->num_rows() > 0) {
$data = array();
$data['receive_items_for_invoice_id'] = $receive_items_for_invoice_id;
...
$this->load->view('receive_items_for_invoice_edit_form', $data);
}
...
if (isset($_POST['receiveditem__orderid'])) {$this->db->where("id !=", $_POST['receive_items_for_invoice_id']);
...
$this->db->where('id', $_POST['receive_items_for_invoice_id']);
$this->db->update('receiveditem', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('receive_items_for_invoiceedit','receiveditem','afteredit', $_POST['receive_items_for_invoice_id']);
