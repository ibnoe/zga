...
class stock_movement_line_viewedit extends Controller {

	function stock_movement_line_viewedit()
	{
		parent::Controller();	
	...
	function index($stock_movement_line_view_id=0)
	{
		if ($stock_movement_line_view_id == 0)
		{
...
$q = $this->db->where('id', $stock_movement_line_view_id);
$this->db->select('*');
$q = $this->db->get('moveactionline');
if ($q->num_rows() > 0) {
$data = array();
$data['stock_movement_line_view_id'] = $stock_movement_line_view_id;
...
$this->load->view('stock_movement_line_view_edit_form', $data);
}
...
$this->db->where('id', $_POST['stock_movement_line_view_id']);
$this->db->update('moveactionline', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('stock_movement_line_viewedit','moveactionline','afteredit', $_POST['stock_movement_line_view_id']);
