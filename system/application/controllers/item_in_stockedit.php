...
class item_in_stockedit extends Controller {

	function item_in_stockedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $item_in_stock_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('item_in_stock_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['item_in_stock_id']);
...
$this->db->where('id', $_POST['item_in_stock_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('item_in_stockedit','item','afteredit', $_POST['item_in_stock_id']);
