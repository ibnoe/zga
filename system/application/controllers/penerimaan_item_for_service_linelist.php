...
class penerimaan_item_for_service_linelist extends Controller {

	function penerimaan_item_for_service_linelist()
	{
		parent::Controller();	
...
$this->db->select('insertitemline.price as insertitemline__price', false);
...
$this->db->select('insertitemline.updatedby as insertitemline__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "warehouse.name like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || insertitemline.quantity like '%".$_POST['searchtext']."%'";$where .= " || uom.name like '%".$_POST['searchtext']."%'";$where .= " || insertitemline.price like '%".$_POST['searchtext']."%'";$where .= " || insertitemline.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || insertitemline.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('insertitemline__lastupdate', 'desc');
	...
	function index($insertitem_id=0)
	{
		...
		$data['foreign_id'] = $insertitem_id;$data['fields'] = array('warehouse__name' => 'Location', 'item__name' => 'Item', 'insertitemline__quantity' => 'Quantity', 'uom__name' => 'Unit', 'insertitemline__price' => 'Value', 'insertitemline__lastupdate' => 'Last Update', 'insertitemline__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('penerimaan_item_for_service_line_list_view', $data);
	}
}
