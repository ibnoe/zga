...
class pay_accountsedit extends Controller {

	function pay_accountsedit()
	{
		parent::Controller();	
	...
	function index($pay_accounts_id=0)
	{
		if ($pay_accounts_id == 0)
		{
...
$q = $this->db->where('id', $pay_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
$data['pay_accounts_id'] = $pay_accounts_id;
...
$this->load->view('pay_accounts_edit_form', $data);
}
...
if (isset($_POST['coa__idstring'])) {$this->db->where("id !=", $_POST['pay_accounts_id']);
...
$this->db->where('id', $_POST['pay_accounts_id']);
$this->db->update('coa', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('pay_accountsedit','coa','afteredit', $_POST['pay_accounts_id']);
