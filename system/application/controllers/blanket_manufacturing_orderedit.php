...
class blanket_manufacturing_orderedit extends Controller {

	function blanket_manufacturing_orderedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $blanket_manufacturing_order_id);
$this->db->select('*');
...
$this->load->view('blanket_manufacturing_order_edit_form', $data);
}
...
if (isset($_POST['manufacturingorder__idstring'])) {$this->db->where("id !=", $_POST['blanket_manufacturing_order_id']);
...
if (!isset($_POST['manufacturingorder__bif_id']) || ($_POST['manufacturingorder__bif_id'] == "" || $_POST['manufacturingorder__bif_id'] == null  || $_POST['manufacturingorder__bif_id'] == 0))
$error .= "<span class='error'>Blanket Identification Form must not be empty"."</span><br>";
...
$error .= "<span class='error'>Blanket must not be empty"."</span><br>";
...
$this->db->set('date', "str_to_date('".$_POST['manufacturingorder__date']."', '%d-%m-%Y')", false);if (isset($_POST['manufacturingorder__bif_id']))
$data['bif_id'] = $_POST['manufacturingorder__bif_id'];if (isset($_POST['manufacturingorder__item_id']))
...
$this->db->where('id', $_POST['blanket_manufacturing_order_id']);
$this->db->update('manufacturingorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('blanket_manufacturing_orderedit','manufacturingorder','afteredit', $_POST['blanket_manufacturing_order_id']);
