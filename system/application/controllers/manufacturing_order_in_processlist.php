...
class manufacturing_order_in_processlist extends Controller {

	function manufacturing_order_in_processlist()
	{
		parent::Controller();	
...
$this->db->where('manufacturingorder.quantityalreadyprocessed > 0');
$this->db->where('manufacturingorder.quantityalreadyprocessed < manufacturingorder.quantity');
...
$this->db->select('manufacturingorder.updatedby as manufacturingorder__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "manufacturingorder.idstring like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.date like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse1.name like '%".$_POST['searchtext']."%'";$where .= " || bom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.quantity like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.quantitytoprocess like '%".$_POST['searchtext']."%'";$where .= " || uom.name like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || manufacturingorder.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('manufacturingorder__lastupdate', 'desc');
	...
	function index($edit_module_id=0)
	{
		...
		$data['fields'] = array('manufacturingorder__idstring' => 'ID', 'manufacturingorder__date' => 'Date', 'item__name' => 'Item', 'warehouse__name' => 'Raw Material Location', 'warehouse1__name' => 'Finish Goods Location', 'bom__name' => 'Bill Of Material', 'manufacturingorder__quantity' => 'Quantity', 'manufacturingorder__quantitytoprocess' => 'Quantity To Process', 'uom__name' => 'Unit', 'manufacturingorder__lastupdate' => 'Last Update', 'manufacturingorder__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('manufacturing_order_in_process_list_view', $data);
	}
}
