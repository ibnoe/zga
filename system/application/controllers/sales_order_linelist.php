...
class sales_order_linelist extends Controller {

	function sales_order_linelist()
	{
		parent::Controller();	
...
$this->db->select('salesorderline.hasppn as salesorderline__hasppn', false);
$this->db->select('salesorderline.pph as salesorderline__pph', false);
...
$this->db->select('salesorderline.updatedby as salesorderline__updatedby', false);
		...
		if (isset($_POST['searchtext']) && $_POST['searchtext'] != "")
		{
			...
			$where .= "salesorderline.type like '%".$_POST['searchtext']."%'";$where .= " || item.name like '%".$_POST['searchtext']."%'";$where .= " || warehouse.name like '%".$_POST['searchtext']."%'";$where .= " || rcn.norcn like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.quantity like '%".$_POST['searchtext']."%'";$where .= " || uom.name like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.price like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.pdisc like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.hasppn like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.pph like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.subtotal like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.lastupdate like '%".$_POST['searchtext']."%'";$where .= " || salesorderline.updatedby like '%".$_POST['searchtext']."%'";
...
$this->db->order_by('salesorderline__lastupdate', 'desc');
	...
	function index($salesorder_id=0)
	{
		...
		$data['foreign_id'] = $salesorder_id;$data['fields'] = array('salesorderline__type' => 'Type', 'item__name' => 'Item', 'warehouse__name' => 'Ship From Location', 'rcn__norcn' => 'RCN', 'salesorderline__quantity' => 'Quantity', 'uom__name' => 'Unit', 'salesorderline__price' => 'Price', 'salesorderline__pdisc' => 'Disc %', 'salesorderline__hasppn' => 'PPN?', 'salesorderline__pph' => 'PPH %', 'salesorderline__subtotal' => 'SubTotal', 'salesorderline__lastupdate' => 'Last Update', 'salesorderline__updatedby' => 'Last Update By');
		...
		///
		$this->load->view('sales_order_line_list_view', $data);
	}
}
