...
class depreciation_accountsview extends Controller {

	function depreciation_accountsview()
	{
		parent::Controller();	
...
$this->db->where('id', $depreciation_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('depreciation_accounts_view_form', $data);
}
...
$this->db->where('id', $data['depreciation_accounts_id']);
