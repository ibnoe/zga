...
class purchaseable_itemview extends Controller {

	function purchaseable_itemview()
	{
		parent::Controller();	
...
$this->db->where('id', $purchaseable_item_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('purchaseable_item_view_form', $data);
}
...
$this->db->where('id', $data['purchaseable_item_id']);
