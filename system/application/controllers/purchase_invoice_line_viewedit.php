...
class purchase_invoice_line_viewedit extends Controller {

	function purchase_invoice_line_viewedit()
	{
		parent::Controller();	
	...
	function index($purchase_invoice_line_view_id=0)
	{
		if ($purchase_invoice_line_view_id == 0)
		{
...
$q = $this->db->where('id', $purchase_invoice_line_view_id);
$q = $this->db->get('purchaseinvoiceline');
if ($q->num_rows() > 0) {
$data = array();
$data['purchase_invoice_line_view_id'] = $purchase_invoice_line_view_id;
...
$this->load->view('purchase_invoice_line_view_edit_form', $data);
}
...
$this->db->where('id', $_POST['purchase_invoice_line_view_id']);
$this->db->update('purchaseinvoiceline', $data);
$this->load->library('generallib');
$this->generallib->commonfunction('purchase_invoice_line_viewedit','purchaseinvoiceline','afteredit', $_POST['purchase_invoice_line_view_id']);
