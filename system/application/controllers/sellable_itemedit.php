...
class sellable_itemedit extends Controller {

	function sellable_itemedit()
	{
		parent::Controller();	
	...
	function index($sellable_item_id=0)
	{
		if ($sellable_item_id == 0)
		{
...
$q = $this->db->where('id', $sellable_item_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
$data['sellable_item_id'] = $sellable_item_id;
...
$this->load->view('sellable_item_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['sellable_item_id']);
...
$this->db->where('id', $_POST['sellable_item_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('sellable_itemedit','item','afteredit', $_POST['sellable_item_id']);
