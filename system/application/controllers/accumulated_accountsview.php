...
class accumulated_accountsview extends Controller {

	function accumulated_accountsview()
	{
		parent::Controller();	
...
$this->db->where('id', $accumulated_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('accumulated_accounts_view_form', $data);
}
...
$this->db->where('id', $data['accumulated_accounts_id']);
