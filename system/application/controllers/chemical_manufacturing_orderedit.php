...
class chemical_manufacturing_orderedit extends Controller {

	function chemical_manufacturing_orderedit()
	{
		parent::Controller();	
	...
	function index($chemical_manufacturing_order_id=0)
	{
		if ($chemical_manufacturing_order_id == 0)
		{
...
$q = $this->db->where('id', $chemical_manufacturing_order_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('manufacturingorder');
if ($q->num_rows() > 0) {
$data = array();
$data['chemical_manufacturing_order_id'] = $chemical_manufacturing_order_id;
foreach ($q->result() as $r) {
$data['manufacturingorder__idstring'] = $r->idstring;
$data['manufacturingorder__date'] = $r->date;
$penambahanstockchemical_opt = array();
$penambahanstockchemical_opt[''] = 'None';
$q = $this->db->get('penambahanstockchemical');
foreach ($q->result() as $row) { $penambahanstockchemical_opt[$row->id] = $row->idstring; }
$data['penambahanstockchemical_opt'] = $penambahanstockchemical_opt;
$data['manufacturingorder__penambahanstockchemical_id'] = $r->penambahanstockchemical_id;
$item_opt = array();
$item_opt[''] = 'None';
$q = $this->db->get('item');
...
$this->load->view('chemical_manufacturing_order_edit_form', $data);
}
...
if (isset($_POST['manufacturingorder__idstring'])) {$this->db->where("id !=", $_POST['chemical_manufacturing_order_id']);
...
if (!isset($_POST['manufacturingorder__penambahanstockchemical_id']) || ($_POST['manufacturingorder__penambahanstockchemical_id'] == "" || $_POST['manufacturingorder__penambahanstockchemical_id'] == null  || $_POST['manufacturingorder__penambahanstockchemical_id'] == 0))
$error .= "<span class='error'>Penambahan Stock Chemical must not be empty"."</span><br>";
...
$this->db->set('date', "str_to_date('".$_POST['manufacturingorder__date']."', '%d-%m-%Y')", false);if (isset($_POST['manufacturingorder__penambahanstockchemical_id']))
$data['penambahanstockchemical_id'] = $_POST['manufacturingorder__penambahanstockchemical_id'];if (isset($_POST['manufacturingorder__item_id']))
...
$this->db->where('id', $_POST['chemical_manufacturing_order_id']);
$this->db->update('manufacturingorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('chemical_manufacturing_orderedit','manufacturingorder','afteredit', $_POST['chemical_manufacturing_order_id']);
