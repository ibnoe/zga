...
class manufacturing_order_waiting_listedit extends Controller {

	function manufacturing_order_waiting_listedit()
	{
		parent::Controller();	
	...
	function index($manufacturing_order_waiting_list_id=0)
	{
		if ($manufacturing_order_waiting_list_id == 0)
		{
...
$q = $this->db->where('id', $manufacturing_order_waiting_list_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('manufacturingorder');
if ($q->num_rows() > 0) {
$data = array();
$data['manufacturing_order_waiting_list_id'] = $manufacturing_order_waiting_list_id;
...
$data['manufacturingorder__quantitytoprocess'] = $r->quantitytoprocess;
$uom_opt = array();
$uom_opt[''] = 'None';
$q = $this->db->get('uom');
...
$this->load->view('manufacturing_order_waiting_list_edit_form', $data);
}
...
if (isset($_POST['manufacturingorder__idstring'])) {$this->db->where("id !=", $_POST['manufacturing_order_waiting_list_id']);
...
$this->db->where('id', $_POST['manufacturing_order_waiting_list_id']);
$this->db->update('manufacturingorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('manufacturing_order_waiting_listedit','manufacturingorder','afteredit', $_POST['manufacturing_order_waiting_list_id']);
