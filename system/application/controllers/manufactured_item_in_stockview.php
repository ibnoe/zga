...
class manufactured_item_in_stockview extends Controller {

	function manufactured_item_in_stockview()
	{
		parent::Controller();	
...
$this->db->where('id', $manufactured_item_in_stock_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('manufactured_item_in_stock_view_form', $data);
}
...
$this->db->where('id', $data['manufactured_item_in_stock_id']);
