...
class depreciation_accountsedit extends Controller {

	function depreciation_accountsedit()
	{
		parent::Controller();	
	...
	function index($depreciation_accounts_id=0)
	{
		if ($depreciation_accounts_id == 0)
		{
...
$q = $this->db->where('id', $depreciation_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
$data['depreciation_accounts_id'] = $depreciation_accounts_id;
...
$this->load->view('depreciation_accounts_edit_form', $data);
}
...
if (isset($_POST['coa__idstring'])) {$this->db->where("id !=", $_POST['depreciation_accounts_id']);
...
$this->db->where('id', $_POST['depreciation_accounts_id']);
$this->db->update('coa', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('depreciation_accountsedit','coa','afteredit', $_POST['depreciation_accounts_id']);
