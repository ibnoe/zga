...
class accessoriesedit extends Controller {

	function accessoriesedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $accessories_id);
$this->db->select('*');
$q = $this->db->get('item');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('accessories_edit_form', $data);
}
...
if (isset($_POST['item__idstring'])) {$this->db->where("id !=", $_POST['accessories_id']);
...
$this->db->where('id', $_POST['accessories_id']);
$this->db->update('item', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('accessoriesedit','item','afteredit', $_POST['accessories_id']);
