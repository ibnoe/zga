...
class sales_return_deliveryedit extends Controller {

	function sales_return_deliveryedit()
	{
		parent::Controller();	
	...
	function index($sales_return_delivery_id=0)
	{
		if ($sales_return_delivery_id == 0)
		{
...
$q = $this->db->where('id', $sales_return_delivery_id);
$this->db->select('*');
$this->db->select('DATE_FORMAT(date, "%d-%m-%Y") as date', false);
$q = $this->db->get('salesreturndelivery');
if ($q->num_rows() > 0) {
$data = array();
$data['sales_return_delivery_id'] = $sales_return_delivery_id;
...
$this->load->view('sales_return_delivery_edit_form', $data);
}
...
$this->db->where('id', $_POST['sales_return_delivery_id']);
$this->db->update('salesreturndelivery', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('sales_return_deliveryedit','salesreturndelivery','afteredit', $_POST['sales_return_delivery_id']);
