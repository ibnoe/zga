...
class manufacturing_order_already_doneedit extends Controller {

	function manufacturing_order_already_doneedit()
	{
		parent::Controller();	
...
$q = $this->db->where('id', $manufacturing_order_already_done_id);
$this->db->select('*');
...
$this->load->view('manufacturing_order_already_done_edit_form', $data);
}
...
if (isset($_POST['manufacturingorder__idstring'])) {$this->db->where("id !=", $_POST['manufacturing_order_already_done_id']);
...
$this->db->where('id', $_POST['manufacturing_order_already_done_id']);
$this->db->update('manufacturingorder', $data);
$this->load->library('generallib');
$error .= $this->generallib->commonfunction('manufacturing_order_already_doneedit','manufacturingorder','afteredit', $_POST['manufacturing_order_already_done_id']);
