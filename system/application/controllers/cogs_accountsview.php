...
class cogs_accountsview extends Controller {

	function cogs_accountsview()
	{
		parent::Controller();	
...
$this->db->where('id', $cogs_accounts_id);
$this->db->select('*');
$q = $this->db->get('coa');
if ($q->num_rows() > 0) {
$data = array();
...
$this->load->view('cogs_accounts_view_form', $data);
}
...
$this->db->where('id', $data['cogs_accounts_id']);
