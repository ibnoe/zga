		...
		<form method="post" action="<?=current_url();?>" id="forwarderlistform" name="forwarderlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/forwarderview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('forwarderview/index/'.$row['id'], $row['forwarder__name']);?></td><td><?=$row['forwarder__address'];?></td><td align='right'><?=number_format($row['forwarder__rating'], 2);?></td><td><?=$row['forwarder__notes'];?></td><td><?=$row['forwarder__lastupdate'];?></td><td><?=$row['forwarder__updatedby'];?></td>
