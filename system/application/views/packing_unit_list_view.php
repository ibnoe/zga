		...
		<form method="post" action="<?=current_url();?>" id="packing_unitlistform" name="packing_unitlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/packing_unitview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('packing_unitview/index/'.$row['id'], $row['packingunit__name']);?></td><td align='right'><?=number_format($row['packingunit__ratio'], 2);?></td><td><?php if (isset($row['packingunit__uom_id']) && $row['uom__name'] != "") echo anchor('uomview/index/'.$row['packingunit__uom_id'], $row['uom__name']);?></td><td><?=$row['packingunit__lastupdate'];?></td><td><?=$row['packingunit__updatedby'];?></td>
