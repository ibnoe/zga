...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#accountsoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#accountseditform').click(function(){$('#accountseditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Accounts</h3>

<p>
<div id="accountsoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/accountsedit/submit" id="accountseditform" class="editform">
