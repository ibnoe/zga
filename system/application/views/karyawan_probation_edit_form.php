...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#karyawan_probationoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#karyawan_probationeditform').click(function(){$('#karyawan_probationeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Karyawan Probation</h3>

<p>
<div id="karyawan_probationoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/karyawan_probationedit/submit" id="karyawan_probationeditform" class="editform">
