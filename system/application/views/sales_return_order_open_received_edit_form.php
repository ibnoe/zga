...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#sales_return_order_open_receivedoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#sales_return_order_open_receivededitform').click(function(){$('#sales_return_order_open_receivededitform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Sales Return Order Open Received</h3>

<p>
<div id="sales_return_order_open_receivedoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/sales_return_order_open_receivededit/submit" id="sales_return_order_open_receivededitform" class="editform">
