		...
		<form method="post" action="<?=current_url();?>" id="currencylistform" name="currencylistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/currencyview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('currencyview/index/'.$row['id'], $row['currency__idstring']);?></td><td><?=$row['currency__name'];?></td><td align='right'><?=number_format($row['currency__rate'], 2);?></td><td><?=$row['currency__lastupdate'];?></td><td><?=$row['currency__updatedby'];?></td>
