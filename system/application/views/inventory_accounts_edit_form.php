...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#inventory_accountsoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#inventory_accountseditform').click(function(){$('#inventory_accountseditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Inventory Accounts</h3>

<p>
<div id="inventory_accountsoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/inventory_accountsedit/submit" id="inventory_accountseditform" class="editform">
