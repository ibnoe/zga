...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#delivery_order_lineoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#delivery_order_lineeditform').click(function(){$('#delivery_order_lineeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Delivery Order Line</h3>

<p>
<div id="delivery_order_lineoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/delivery_order_lineedit/submit" id="delivery_order_lineeditform" class="editform">
