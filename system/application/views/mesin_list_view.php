		...
		<form method="post" action="<?=current_url();?>" id="mesinlistform" name="mesinlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/mesinview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('mesinview/index/'.$row['id'], $row['mesin__idstring']);?></td><td><?=$row['mesin__typename'];?></td><td><?php if (isset($row['mesin__merkmesin_id']) && $row['merkmesin__name'] != "") echo anchor('merk_mesinview/index/'.$row['mesin__merkmesin_id'], $row['merkmesin__name']);?></td><td><?=$row['mesin__lastupdate'];?></td><td><?=$row['mesin__updatedby'];?></td>
