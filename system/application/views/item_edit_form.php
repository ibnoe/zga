...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#itemoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#itemeditform').click(function(){$('#itemeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Item</h3>

<p>
<div id="itemoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/itemedit/submit" id="itemeditform" class="editform">
...
<td>Category *</td><td><?=form_dropdown('item__itemcategory_id', $itemcategory_opt, $item__itemcategory_id);?>&nbsp;<input id='item__itemcategory_id_lookup' type='button' value='Lookup'></input></td><div id='item__itemcategory_id_dialog'></div><script type="text/javascript">$(document).ready(function() {$('#item__itemcategory_id_dialog').dialog({ autoOpen: false, height: 500, width: 700, modal: true, buttons: { Cancel: function() { $( this ).dialog( 'close' ); } }, });$.get('<?=site_url();?>/item_categorylookup', function(data) { $('#item__itemcategory_id_dialog').html(data);$('#item__itemcategory_id_dialog table tr').live('click', function() { var tr = $(this);var lines = $('td', tr).map(function(index, td) { return $(td).text(); });if (lines[0] != null) {$('select[name=item__itemcategory_id]').append('<option value=' + lines[0] + '>' + lines[1] + '</option>');$('select[name=item__itemcategory_id]').val(lines[0]);if (typeof window.item_selected_itemcategory_id == 'function') { item_selected_itemcategory_id("<?=site_url();?>"); }}$('#item__itemcategory_id_dialog').dialog('close');});$('#item__itemcategory_id_lookup').button().click(function() {$('#item__itemcategory_id_dialog').dialog('open');});});});</script></tr><tr class='basic'>
