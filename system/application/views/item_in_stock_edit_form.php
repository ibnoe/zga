...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#item_in_stockoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#item_in_stockeditform').click(function(){$('#item_in_stockeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Item In Stock</h3>

<p>
<div id="item_in_stockoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/item_in_stockedit/submit" id="item_in_stockeditform" class="editform">
