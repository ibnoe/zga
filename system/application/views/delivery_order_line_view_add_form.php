...
<script type="text/javascript">
  $(document).ready(function() {
    var options = { 
			target:        '#delivery_order_line_viewoutput',
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		}; 
		
		$('#delivery_order_line_viewform').click(function(){$('#delivery_order_line_viewform').ajaxForm(options);});
...
<h3 class="addtitle">New Delivery Order Line View</h3>

<p>
<div id="delivery_order_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/delivery_order_line_viewadd/submit" id="delivery_order_line_viewform" class="addform">

<table width="100%" class="addtable">
<?=form_hidden('deliveryorder_id', $deliveryorder_id);?>
<tr class='basic'>
<td>Item *</td>
