<html>
<head><title>Laporan Penjualan Quantity Per Marketing Tahunan | tahun <?=$year;?></title>
...
<div class="reportheader">
	...
	<h1>Laporan Penjualan Quantity Per Marketing Tahunan</h1>
...
foreach ($q->result() as $row)
{
	...
	<?php foreach ($col_arr as $k=>$v): ?>
		<?php $tcol[$k] = 0; ?>
	<?php endforeach; ?>
	<?php foreach ($row_arr as $marketingofficer_id=>$v): ?>
		<tr>
		<td><?=$v;?></td>
		<?php $trow = 0; ?>
		<?php foreach ($col_arr as $month=>$v): ?>
			...
			<?php
			{
                ...
                //$this->db->join('deliveryorderline', 'deliveryorderline.deliveryorder_id = deliveryorder.id');
                //$this->db->join('salesorderline', 'deliveryorderline.salesorderline_id = salesorderline.id');
                ...
                $this->db->select('salesorder.currency_id');
                $this->db->select('salesorder.marketingofficer_id');
                $q = $this->db->get();

				foreach ($q->result() as $row)
				{
					if ($row->currency_id == $currency_id)
						if ($row->marketingofficer_id == $marketingofficer_id)
						{
							$date = $row->date;
							...
							if ($year == $y)
							{
								if ($m == $month)
								{
									$total += $row->quantity * $row->multiplier;
								}
							}
						}
				}
