...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#sales_order_open_sentoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#sales_order_open_senteditform').click(function(){$('#sales_order_open_senteditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Sales Order Open Sent</h3>

<p>
<div id="sales_order_open_sentoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/sales_order_open_sentedit/submit" id="sales_order_open_senteditform" class="editform">

<?=form_hidden("sales_order_open_sent_id", $sales_order_open_sent_id);?>

<table width="100%" class="edittable">
<tr class='basic'>
<td>SO ID *</td><td><?=form_input(array('name' => 'salesorder__orderid', 'value' => $salesorder__orderid, 'id' => 'salesorder__orderid'));?></td></tr><tr class='basic'><script type="text/javascript">$(document).ready(function() {$("#salesorder__date").datepicker({ showOn: 'button', buttonText: "Cal", autoSize: true, dateFormat: 'dd-mm-yy' });});</script>
