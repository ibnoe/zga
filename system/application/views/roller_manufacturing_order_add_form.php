...
<script type="text/javascript">
  $(document).ready(function() {
    var options = { 
			target:        '#roller_manufacturing_orderoutput',
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=site_url();?>/roller_manufacturing_orderview/index/' },
		}; 
		
		$('#roller_manufacturing_orderform').click(function(){$('#roller_manufacturing_orderform').ajaxForm(options);});
...
<h3 class="addtitle">New Roller Manufacturing Order</h3>

<p>
<div id="roller_manufacturing_orderoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/roller_manufacturing_orderadd/submit" id="roller_manufacturing_orderform" class="addform">
...
<tr class='basic'>
<td>RCN *</td>
<td><?=form_dropdown('manufacturingorder__rcn_id', array(), '', 'class="basic"');?>&nbsp;<input id='manufacturingorder__rcn_id_lookup' type='button' value='Lookup'></input></td><div id='manufacturingorder__rcn_id_dialog'></div><script type="text/javascript">$(document).ready(function() {$('#manufacturingorder__rcn_id_dialog').dialog({ autoOpen: false, height: 500, width: 700, modal: true, buttons: { Cancel: function() { $( this ).dialog( 'close' ); } }, });$.get('<?=site_url();?>/rcnlookup', function(data) { $('#manufacturingorder__rcn_id_dialog').html(data);$('#manufacturingorder__rcn_id_dialog a').attr('disabled', 'disabled');$('#manufacturingorder__rcn_id_dialog table tr').live('click', function() { var tr = $(this);var lines = $('td', tr).map(function(index, td) { return $(td).text(); });if (lines[0] != null) {$('select[name=manufacturingorder__rcn_id]').append('<option value=' + lines[0] + '>' + lines[2] + '</option>');$('select[name=manufacturingorder__rcn_id]').val(lines[0]);if (typeof window.roller_manufacturing_order_selected_rcn_id == 'function') { roller_manufacturing_order_selected_rcn_id("<?=site_url();?>"); }}$('#manufacturingorder__rcn_id_dialog').dialog('close');});$('#manufacturingorder__rcn_id_lookup').button().click(function() {$('#manufacturingorder__rcn_id_dialog').dialog('open');});});});</script></tr>
<tr class='basic'>
<td>Roll *</td>
