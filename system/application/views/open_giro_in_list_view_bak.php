		...
		<form method="post" action="<?=current_url();?>" id="open_giro_inlistform" name="open_giro_inlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td class='hidden'><?=$row['id'];?></td><td><?= form_checkbox('giroin__id[]', $row['giroin__id'], false);?></td><td><?=$row['giroin__createdate'];?></td><td><?=$row['giroin__giroinid'];?></td><td><?php if (isset($row['giroin__customer_id']) && $row['customer__firstname'] != "") echo anchor('customerview/index/'.$row['giroin__customer_id'], $row['customer__firstname']);?></td><td><?php if (isset($row['giroin__currency_id']) && $row['currency__name'] != "") echo anchor('currencyview/index/'.$row['giroin__currency_id'], $row['currency__name']);?></td><td><?=number_format($row['giroin__amount'], 2);?></td><td><?=number_format($row['giroin__amountused'], 2);?></td><td><?=$row['giroin__lastupdate'];?></td><td><?=$row['giroin__updatedby'];?></td>
			...
			<script type="text/javascript">
			$(document).ready(function() {
			$('#giro_flagused').click(function(){
				var data = $('#open_giro_inlistform').serialize();
				$.ajax({
				  type: 'POST',
				  url: '<?=site_url();?>/giroin_flagused',
				  data: data,
			...
			<input id='giro_flagused' type="submit" value="Mark As Used">
