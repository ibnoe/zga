...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#sales_return_invoice_line_viewoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#sales_return_invoice_line_vieweditform').click(function(){$('#sales_return_invoice_line_vieweditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Sales Return Invoice Line View</h3>

<p>
<div id="sales_return_invoice_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/sales_return_invoice_line_viewedit/submit" id="sales_return_invoice_line_vieweditform" class="editform">
