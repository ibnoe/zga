...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#consumableoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#consumableeditform').click(function(){$('#consumableeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Consumable</h3>

<p>
<div id="consumableoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/consumableedit/submit" id="consumableeditform" class="editform">
