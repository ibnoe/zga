...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#blanket_manufacturing_orderoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#blanket_manufacturing_ordereditform').click(function(){$('#blanket_manufacturing_ordereditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Blanket Manufacturing Order</h3>

<p>
<div id="blanket_manufacturing_orderoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/blanket_manufacturing_orderedit/submit" id="blanket_manufacturing_ordereditform" class="editform">
...
<td>Blanket Identification Form *</td><td><?=form_dropdown('manufacturingorder__bif_id', $bif_opt, $manufacturingorder__bif_id);?>&nbsp;<input id='manufacturingorder__bif_id_lookup' type='button' value='Lookup'></input></td><div id='manufacturingorder__bif_id_dialog'></div><script type="text/javascript">$(document).ready(function() {$('#manufacturingorder__bif_id_dialog').dialog({ autoOpen: false, height: 500, width: 700, modal: true, buttons: { Cancel: function() { $( this ).dialog( 'close' ); } }, });$.get('<?=site_url();?>/blanket_identification_formlookup', function(data) { $('#manufacturingorder__bif_id_dialog').html(data);$('#manufacturingorder__bif_id_dialog table tr').live('click', function() { var tr = $(this);var lines = $('td', tr).map(function(index, td) { return $(td).text(); });if (lines[0] != null) {$('select[name=manufacturingorder__bif_id]').append('<option value=' + lines[0] + '>' + lines[1] + '</option>');$('select[name=manufacturingorder__bif_id]').val(lines[0]);if (typeof window.blanket_manufacturing_order_selected_bif_id == 'function') { blanket_manufacturing_order_selected_bif_id("<?=site_url();?>"); }}$('#manufacturingorder__bif_id_dialog').dialog('close');});$('#manufacturingorder__bif_id_lookup').button().click(function() {$('#manufacturingorder__bif_id_dialog').dialog('open');});});});</script></tr><tr class='basic'>
<td>Blanket *</td><td><?=form_dropdown('manufacturingorder__item_id', $item_opt, $manufacturingorder__item_id);?>&nbsp;<input id='manufacturingorder__item_id_lookup' type='button' value='Lookup'></input></td><div id='manufacturingorder__item_id_dialog'></div><script type="text/javascript">$(document).ready(function() {$('#manufacturingorder__item_id_dialog').dialog({ autoOpen: false, height: 500, width: 700, modal: true, buttons: { Cancel: function() { $( this ).dialog( 'close' ); } }, });$.get('<?=site_url();?>/manufactured_itemlookup', function(data) { $('#manufacturingorder__item_id_dialog').html(data);$('#manufacturingorder__item_id_dialog table tr').live('click', function() { var tr = $(this);var lines = $('td', tr).map(function(index, td) { return $(td).text(); });if (lines[0] != null) {$('select[name=manufacturingorder__item_id]').append('<option value=' + lines[0] + '>' + lines[2] + '</option>');$('select[name=manufacturingorder__item_id]').val(lines[0]);if (typeof window.blanket_manufacturing_order_selected_item_id == 'function') { blanket_manufacturing_order_selected_item_id("<?=site_url();?>"); }}$('#manufacturingorder__item_id_dialog').dialog('close');});$('#manufacturingorder__item_id_lookup').button().click(function() {$('#manufacturingorder__item_id_dialog').dialog('open');});});});</script></tr><tr class='basic'>
