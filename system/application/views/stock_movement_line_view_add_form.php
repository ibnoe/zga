...
<script type="text/javascript">
  $(document).ready(function() {
    var options = { 
			target:        '#stock_movement_line_viewoutput',
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		}; 
		
		$('#stock_movement_line_viewform').click(function(){$('#stock_movement_line_viewform').ajaxForm(options);});
...
<h3 class="addtitle">New Stock Movement Line View</h3>

<p>
<div id="stock_movement_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/stock_movement_line_viewadd/submit" id="stock_movement_line_viewform" class="addform">

<table width="100%" class="addtable">
<?=form_hidden('moveaction_id', $moveaction_id);?>
<tr class='basic'>
<td>Item *</td>
