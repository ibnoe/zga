...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#suar_pengajuan_repairoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#suar_pengajuan_repaireditform').click(function(){$('#suar_pengajuan_repaireditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Suar Pengajuan Repair</h3>

<p>
<div id="suar_pengajuan_repairoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/suar_pengajuan_repairedit/submit" id="suar_pengajuan_repaireditform" class="editform">
