...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#supplieroutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#suppliereditform').click(function(){$('#suppliereditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Supplier</h3>

<p>
<div id="supplieroutput"></div>
</p>

<form method="post" action="<?=site_url();?>/supplieredit/submit" id="suppliereditform" class="editform">
