		...
		<form method="post" action="<?=current_url();?>" id="credit_note_outlistform" name="credit_note_outlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td class='hidden'><?=$row['id'];?></td><td><?=$row['creditnoteout__creditnoteoutid'];?></td><td><?=$row['creditnoteout__date'];?></td><td><?=$row['creditnoteout__expirydate'];?></td><td><?php if (isset($row['creditnoteout__customer_id']) && $row['creditnoteout__customer_id'] > 0) echo $row['customer__firstname'];?></td><td><?php if (isset($row['creditnoteout__coa_id']) && $row['creditnoteout__coa_id'] > 0) echo $row['coa__name'];?></td><td><?php if (isset($row['creditnoteout__currency_id']) && $row['creditnoteout__currency_id'] > 0) echo $row['currency__name'];?></td><td align='right'><?=number_format($row['creditnoteout__amount'], 2);?></td><td align='right'><?=number_format($row['creditnoteout__amountused'], 2);?></td><td><?=$row['creditnoteout__notes'];?></td><td><?php if ($row['creditnoteout__usedflag'] != 0) echo 'Yes'; else echo '';?></td><td><?=$row['creditnoteout__lastupdate'];?></td><td><?=$row['creditnoteout__updatedby'];?></td>
