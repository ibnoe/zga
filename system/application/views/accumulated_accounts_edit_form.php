...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#accumulated_accountsoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#accumulated_accountseditform').click(function(){$('#accumulated_accountseditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Accumulated Accounts</h3>

<p>
<div id="accumulated_accountsoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/accumulated_accountsedit/submit" id="accumulated_accountseditform" class="editform">
