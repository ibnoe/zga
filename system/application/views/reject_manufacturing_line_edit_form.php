...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#reject_manufacturing_lineoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#reject_manufacturing_lineeditform').click(function(){$('#reject_manufacturing_lineeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Reject Manufacturing Line</h3>

<p>
<div id="reject_manufacturing_lineoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/reject_manufacturing_lineedit/submit" id="reject_manufacturing_lineeditform" class="editform">
