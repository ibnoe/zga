...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#delivery_orderoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#delivery_ordereditform').click(function(){$('#delivery_ordereditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Delivery Order</h3>

<p>
<div id="delivery_orderoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/delivery_orderedit/submit" id="delivery_ordereditform" class="editform">
