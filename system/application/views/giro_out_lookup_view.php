		...
		<form method="post" action="<?=current_url();?>" id="giro_outlistform" name="giro_outlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td class='hidden'><?=$row['id'];?></td><td><?=$row['giroout__girooutid'];?></td><td><?=$row['giroout__createdate'];?></td><td><?php if (isset($row['giroout__supplier_id']) && $row['giroout__supplier_id'] > 0) echo $row['supplier__firstname'];?></td><td><?php if (isset($row['giroout__currency_id']) && $row['giroout__currency_id'] > 0) echo $row['currency__name'];?></td><td align='right'><?=number_format($row['giroout__amount'], 2);?></td><td align='right'><?=number_format($row['giroout__amountused'], 2);?></td><td><?php if (isset($row['giroout__coa_id']) && $row['giroout__coa_id'] > 0) echo $row['coa__name'];?></td><td><?=$row['giroout__notes'];?></td><td><?php if ($row['giroout__usedflag'] != 0) echo 'Yes'; else echo '';?></td><td><?=$row['giroout__lastupdate'];?></td><td><?=$row['giroout__updatedby'];?></td>
