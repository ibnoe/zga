...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#purchase_invoice_lineoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#purchase_invoice_lineeditform').click(function(){$('#purchase_invoice_lineeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Purchase Invoice Line</h3>

<p>
<div id="purchase_invoice_lineoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/purchase_invoice_lineedit/submit" id="purchase_invoice_lineeditform" class="editform">
