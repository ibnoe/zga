...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#pay_accountsoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#pay_accountseditform').click(function(){$('#pay_accountseditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Pay Accounts</h3>

<p>
<div id="pay_accountsoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/pay_accountsedit/submit" id="pay_accountseditform" class="editform">
