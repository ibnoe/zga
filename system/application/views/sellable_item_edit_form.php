...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#sellable_itemoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#sellable_itemeditform').click(function(){$('#sellable_itemeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Sellable Item</h3>

<p>
<div id="sellable_itemoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/sellable_itemedit/submit" id="sellable_itemeditform" class="editform">
