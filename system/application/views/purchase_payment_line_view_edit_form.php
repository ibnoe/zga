...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#purchase_payment_line_viewoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#purchase_payment_line_vieweditform').click(function(){$('#purchase_payment_line_vieweditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Purchase Payment Line View</h3>

<p>
<div id="purchase_payment_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/purchase_payment_line_viewedit/submit" id="purchase_payment_line_vieweditform" class="editform">
