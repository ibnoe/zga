...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#stock_movement_lineoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#stock_movement_lineeditform').click(function(){$('#stock_movement_lineeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Stock Movement Line</h3>

<p>
<div id="stock_movement_lineoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/stock_movement_lineedit/submit" id="stock_movement_lineeditform" class="editform">
