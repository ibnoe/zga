...
<script type="text/javascript">
  $(document).ready(function() {
    var options = { 
			target:        '#reject_manufacturing_line_viewoutput',
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		}; 
		
		$('#reject_manufacturing_line_viewform').click(function(){$('#reject_manufacturing_line_viewform').ajaxForm(options);});
...
<h3 class="addtitle">New Reject Manufacturing Line View</h3>

<p>
<div id="reject_manufacturing_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/reject_manufacturing_line_viewadd/submit" id="reject_manufacturing_line_viewform" class="addform">

<table width="100%" class="addtable">
<?=form_hidden('rejectmanufacturing_id', $rejectmanufacturing_id);?>
<tr class='basic'>
<td>Item *</td>
