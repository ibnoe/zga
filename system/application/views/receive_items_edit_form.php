...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#receive_itemsoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#receive_itemseditform').click(function(){$('#receive_itemseditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Receive Items</h3>

<p>
<div id="receive_itemsoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/receive_itemsedit/submit" id="receive_itemseditform" class="editform">
