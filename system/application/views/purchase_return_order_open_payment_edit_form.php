...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#purchase_return_order_open_paymentoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#purchase_return_order_open_paymenteditform').click(function(){$('#purchase_return_order_open_paymenteditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Purchase Return Order Open Payment</h3>

<p>
<div id="purchase_return_order_open_paymentoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/purchase_return_order_open_paymentedit/submit" id="purchase_return_order_open_paymenteditform" class="editform">
