		...
		<form method="post" action="<?=current_url();?>" id="accountslistform" name="accountslistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/accountsview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('accountsview/index/'.$row['id'], $row['coa__idstring']);?></td><td><?=$row['coa__name'];?></td><td><?php if (isset($row['coa__coatype_id']) && $row['coatype__name'] != "") echo anchor('account_typeview/index/'.$row['coa__coatype_id'], $row['coatype__name']);?></td><td><?=$row['coa__lastupdate'];?></td><td><?=$row['coa__updatedby'];?></td>
