...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#rolloutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#rolleditform').click(function(){$('#rolleditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Roll</h3>

<p>
<div id="rolloutput"></div>
</p>

<form method="post" action="<?=site_url();?>/rolledit/submit" id="rolleditform" class="editform">
