...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#manufactured_itemoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#manufactured_itemeditform').click(function(){$('#manufactured_itemeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Manufactured Item</h3>

<p>
<div id="manufactured_itemoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/manufactured_itemedit/submit" id="manufactured_itemeditform" class="editform">
