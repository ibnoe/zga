		...
		<form method="post" action="<?=current_url();?>" id="bill_of_materiallistform" name="bill_of_materiallistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/bill_of_materialview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('bill_of_materialview/index/'.$row['id'], $row['bom__name']);?></td><td><?=$row['bom__lastupdate'];?></td><td><?=$row['bom__updatedby'];?></td>
