...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#purchase_return_payment_lineoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#purchase_return_payment_lineeditform').click(function(){$('#purchase_return_payment_lineeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Purchase Return Payment Line</h3>

<p>
<div id="purchase_return_payment_lineoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/purchase_return_payment_lineedit/submit" id="purchase_return_payment_lineeditform" class="editform">
