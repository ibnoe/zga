...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#sales_return_delivery_for_invoiceoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#sales_return_delivery_for_invoiceeditform').click(function(){$('#sales_return_delivery_for_invoiceeditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Sales Return Delivery For Invoice</h3>

<p>
<div id="sales_return_delivery_for_invoiceoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/sales_return_delivery_for_invoiceedit/submit" id="sales_return_delivery_for_invoiceeditform" class="editform">
