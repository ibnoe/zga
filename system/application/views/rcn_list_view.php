		...
		<form method="post" action="<?=current_url();?>" id="rcnlistform" name="rcnlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/rcnview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('rcnview/index/'.$row['id'], $row['rcn__norif']);?></td><td><?=$row['rcn__norcn'];?></td><td><?=$row['rcn__customerponumber'];?></td><td><?=$row['rcn__datercn'];?></td><td><?php if (isset($row['rcn__customer_id']) && $row['customer__firstname'] != "") echo anchor('customerview/index/'.$row['rcn__customer_id'], $row['customer__firstname']);?></td><td><?php if ($row['rcn__reqtorecover'] != 0) echo 'Yes'; else echo '';?></td><td><?php if ($row['rcn__reqcoretoreturn'] != 0) echo 'Yes'; else echo '';?></td><td><?php if ($row['rcn__reqreturnunused'] != 0) echo 'Yes'; else echo '';?></td><td><?php if ($row['rcn__reqreturnfaulty'] != 0) echo 'Yes'; else echo '';?></td><td><?php if ($row['rcn__reqothers'] != 0) echo 'Yes'; else echo '';?></td><td><?=$row['rcn__notes'];?></td><td><?=$row['rcn__status'];?></td><td align='right'><?=number_format($row['rcn__totalrollerscollected'], 2);?></td><td><?=$row['rcn__lastupdate'];?></td><td><?=$row['rcn__updatedby'];?></td>
