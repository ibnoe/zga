...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var options = {
			target:        '#giro_out_clearance_line_viewoutput',   // target element(s) to be updated with server response 
			success: 		function(data) { if (data.indexOf('success') != -1) location.href='<?=$_SERVER['HTTP_REFERER'];?>' },
		};
			 
		$('#giro_out_clearance_line_vieweditform').click(function(){$('#giro_out_clearance_line_vieweditform').ajaxForm(options);});
	});
</script>

<div id="maincontent">

<h3 class="edittitle">Edit Giro Out Clearance Line View</h3>

<p>
<div id="giro_out_clearance_line_viewoutput"></div>
</p>

<form method="post" action="<?=site_url();?>/giro_out_clearance_line_viewedit/submit" id="giro_out_clearance_line_vieweditform" class="editform">
