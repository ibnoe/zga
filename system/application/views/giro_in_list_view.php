		...
		<form method="post" action="<?=current_url();?>" id="giro_inlistform" name="giro_inlistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/giro_inview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('giro_inview/index/'.$row['id'], $row['giroin__giroinid']);?></td><td><?=$row['giroin__createdate'];?></td><td><?php if (isset($row['giroin__customer_id']) && $row['customer__firstname'] != "") echo anchor('customerview/index/'.$row['giroin__customer_id'], $row['customer__firstname']);?></td><td><?php if (isset($row['giroin__currency_id']) && $row['currency__name'] != "") echo anchor('currencyview/index/'.$row['giroin__currency_id'], $row['currency__name']);?></td><td align='right'><?=number_format($row['giroin__amount'], 2);?></td><td align='right'><?=number_format($row['giroin__amountused'], 2);?></td><td><?php if (isset($row['giroin__coa_id']) && $row['coa__name'] != "") echo anchor('accountsview/index/'.$row['giroin__coa_id'], $row['coa__name']);?></td><td><?=$row['giroin__notes'];?></td><td><?php if ($row['giroin__usedflag'] != 0) echo 'Yes'; else echo '';?></td><td><?=$row['giroin__lastupdate'];?></td><td><?=$row['giroin__updatedby'];?></td>
