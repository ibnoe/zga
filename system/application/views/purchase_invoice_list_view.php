		...
		<form method="post" action="<?=current_url();?>" id="purchase_invoicelistform" name="purchase_invoicelistform" class="listform">
			...
			<table class="main">
				...
				<?php foreach ($rows as $row): ?>
					...
					<td><a href="<?=base_url().'index.php/purchase_invoiceview/index/'.$row['id'];?>"><img src="<?=base_url();?>resources/detail.png"></img></a></td>
					<td class='hidden'><?=$row['id'];?></td><td><?=anchor('purchase_invoiceview/index/'.$row['id'], $row['purchaseinvoice__date']);?></td><td><?=$row['purchaseinvoice__orderid'];?></td><td><?php if (isset($row['purchaseinvoice__receiveditem_id']) && $row['receiveditem__suratjalanno'] != "") echo anchor('receive_items_for_invoiceview/index/'.$row['purchaseinvoice__receiveditem_id'], $row['receiveditem__suratjalanno']);?></td><td align='right'><?=number_format($row['purchaseinvoice__total'], 2);?></td><td><?=$row['purchaseinvoice__top'];?></td><td><?php if (isset($row['purchaseinvoice__ongkoskirimimport_id']) && $row['ongkoskirimimport__idstring'] != "") echo anchor('ongkos_kirim_importview/index/'.$row['purchaseinvoice__ongkoskirimimport_id'], $row['ongkoskirimimport__idstring']);?></td><td><?=$row['purchaseinvoice__lastupdate'];?></td><td><?=$row['purchaseinvoice__updatedby'];?></td>
